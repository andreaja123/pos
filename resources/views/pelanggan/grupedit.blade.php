...
@section('title', 'Edit Grup Pelanggan - Hanania POS')
            ...
            <div class="bg-white dark:bg-darkCard p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-lg">
                <div class="space-y-4">
                    <div>
                        ...
                        <input name="name" value="{{ old('name', $group->name) }}" class="w-full px-4 py-3 rounded-xl border" required />
                        @error('name')<p class="text-sm text-red-600 mt-2">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="block text-sm font-bold mb-2">Deskripsi</label>
                        <textarea name="description" class="w-full px-4 py-3 rounded-xl" rows="3">{{ old('description', $group->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="discount_percentage" value="{{ old('discount_percentage', $group->discount_percentage) }}" class="w-full px-4 py-3 rounded-xl" />
                            @error('discount_percentage')<p class="text-sm text-red-600 mt-2">{{ $message }}</p>@enderror
                        </div>
                        <div class="flex items-center gap-3">
                            <input type="checkbox" name="active" value="1" {{ old('active', $group->active) ? 'checked' : '' }} />
