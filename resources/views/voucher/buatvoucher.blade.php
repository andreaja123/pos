...
@section('title', 'Buat Voucher - Hanania POS')

@section('content')
    <section id="create-voucher" class="page-section active max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">
                    Tambahkan promo atau kupon diskon untuk pelanggan.
                </p>
            </div>
            <div class="flex gap-3">
                <button onclick="history.back()"
                    class="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition">
                    Batal
                </button>
            </div>
        </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="coupon_name" value="{{ old('coupon_name') }}" placeholder="Contoh: DISKON MERDEKA"
                                ...
                                <p class="text-xs text-slate-400 mt-1">Kode unik yang akan digunakan pelanggan.</p>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" name="coupon_value" value="{{ old('coupon_value') }}" placeholder="0"
                                            class="w-full pl-11 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition placeholder-slate-400 dark:placeholder-slate-500"
                                            required>
                                        @error('coupon_value')
                                            <p class="text-xs text-red-500 mt-2">{{ $message }}</p>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" name="quantity" value="{{ old('quantity', 1) }}" placeholder="100"
                                            class="w-full pl-11 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition placeholder-slate-400 dark:placeholder-slate-500"
                                            required>
                                        @error('quantity')
                                            <p class="text-xs text-red-500 mt-2">{{ $message }}</p>
                            ...
                            <div
                                ...
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" name="link_account" value="1" class="sr-only peer">
                                        <div
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <select name="account_id" id="account_id" {{ old('link_account') ? '' : 'disabled' }}
                                            ...
                                            @foreach($customers as $cust)
                                                <option value="{{ $cust->id }}" {{ old('account_id') == $cust->id ? 'selected' : '' }}>{{ $cust->name }} {{ $cust->email ? '('.$cust->email.')' : '' }}</option>
                                            @endforeach
                                        </select>
                                        @error('account_id')
                                            <p class="text-xs text-red-500 mt-2">{{ $message }}</p>
